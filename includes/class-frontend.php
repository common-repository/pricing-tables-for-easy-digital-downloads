...
class PTF_Pricing_Tables_For_EDD_Frontend {

	public function __construct() {
		add_action( 'pricing_tables_for_edd_footer_end', array( $this, 'purchase_button' ) );
	}

	/**
	 * Build the pricing table
	 */
	public function ptf_pricing_tables( $args = array() ) {
		...
		add_filter( 'edd_' . strtolower( edd_get_currency() ) . '_currency_filter_before', 'pricing_tables_for_edd_filter_currency', 10, 3 );
	...
	public function pricing_table_single( $download_id ) {
		...
		$pricing_table_enabled = get_post_meta( $download_id, '_ptf_pricing_tables', true );
		...
		$classes = array( 'ptf-pricing-tables' );
		$count   = count( $price_options );
		$has_featured = pricing_tables_for_edd_has_featured( $download_id );

		$classes[] = 'ptf-pricing-tables-columns-' . $count;
		...
		<?php if ( $price_options ) : ?>
			...
			<?php foreach ( $price_options as $price_id => $price_option ) :
				...
				$list_item_mode = apply_filters( 'pricing_tables_for_edd_list_item_mode', true );
				...
				$icon                = ! empty( $price_option['pricing_option_icon'] ) ? $price_option['pricing_option_icon'] : '';

				// Button text.
				$button_text = ! empty( $price_option['button_text'] ) ? $price_option['button_text'] : __( 'Purchase', 'pricing-tables-for-easy-digital-downloads' );
				...
				?>
				 ...
				 <div class="<?php echo implode( ' ', array_filter( $classes ) ); ?>">
					 <div class="ptf-pricing-tables-inr">
						<?php if ( $is_featured ) : ?>
							<span class="featured-text"><?php _e('Featured', 'pricing-tables-for-easy-digital-downloads') ?></span>
						<?php endif; ?>

						<span class="edd-pt-title"><?php echo esc_attr( $title ); ?></span>

						<?php do_action( 'pricing_tables_for_edd_after_title', $args ); ?>
						...
						<?php if ( $icon ) : ?>
							<span class="edd-pt-icon"><i class="fa <?php echo esc_attr( $icon ); ?>"></i></span>
						<?php endif ?>
						<?php do_action( 'pricing_tables_for_edd_after_icon', $args ); ?>
						...
						<ul>
							...
							<li class="pricing">
								...
								<?php do_action( 'pricing_tables_for_edd_pricing_end', $args ); ?>
							</li>

							<?php do_action( 'pricing_tables_for_edd_features_start', $price_id ); ?>
							...
							<?php if ( $list_item_mode ) : $i = 0; ?>
								...
								<?php do_action( 'pricing_tables_for_edd_after_feature_' . $i, $price_id ); ?>
							...
							<?php do_action( 'pricing_tables_for_edd_features_end', $price_id ); ?>

						</ul>

						<div class="footer">
							<?php do_action( 'pricing_tables_for_edd_footer_end', $args ); ?>
						</div>
	...
	public function pricing_table_multiple( $download_ids ) {
		...
		if ( $download_ids ) {
			foreach ( $download_ids as $key => $download_id ) {
				...
				if ( ! get_post_meta( $download_id, '_ptf_pricing_tables', true ) ) {
				...
				if ( pricing_tables_for_edd_has_featured( $download_id ) ) {
					$has_featured = true;
					break;
				} else {
					$has_featured = false;
		...
		<?php if ( $download_ids ) : ?>
			...
			<?php foreach ( $download_ids as $key => $download_id ) : ?>
				...
				// Period.
				$period = get_post_meta( $download_id, '_pricing_tables_for_edd_option_period', true );

				// Description.
				$pricing_description = get_post_meta( $download_id, '_pricing_tables_for_edd_option_description', true );
				...
				$title = get_post_meta( $download_id, '_pricing_tables_for_edd_option_name', true );
				$title = $title ? esc_attr( $title ) :  esc_attr( $download->post_title );

				// Icon
				$icon = get_post_meta( $download_id, '_pricing_tables_for_edd_option_icon', true );
				$icon = $icon ? esc_attr( $icon ) :  esc_attr( $download->post_icon );

				// Custom button text.
				$button_text = get_post_meta( $download_id, '_pricing_tables_for_edd_button_text', true );
				$button_text = ! empty( $button_text ) ? $button_text : __( 'Purchase', 'pricing-tables-for-easy-digital-downloads' );
				...
				<div class="edd-price-option<?php echo $is_featured; ?>">
					<div class="ptf-pricing-tables-inr">
						...
						<ul>
							...
							<?php do_action( 'pricing_tables_for_edd_features_start', $args ); ?>

							<?php if ( $list_item_mode ) : $i = 0; ?>

							<?php foreach ( $features as $feature ) : $i++; ?>
								<li class="edd-pt-feature<?php echo ' edd-pt-feature-' . $i; ?>"><?php echo $feature; ?></li>
								<?php do_action( 'pricing_tables_for_edd_after_feature_' . $i, $download_id ); ?>
							<?php endforeach; ?>
							...
							<?php do_action( 'pricing_tables_for_edd_features_end', $args ); ?>
	...
	public function purchase_button( $args = array() ) {
		...
		$button_text = $in_cart ? apply_filters( 'pricing_tables_for_edd_complete_purchase_text', __( 'Checkout', 'pricing-tables-for-easy-digital-downloads' ) ) : $args['button_text'];
